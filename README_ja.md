...
* **データの保存**: Slack の会話内容はデータベースに保存されません。メモリ上で一時的に処理され、ADR生成のための要約にのみ使用されます。
* **機密情報の取り扱い**: 入力内容は AI モデルに送信されます。個人情報や機密情報が含まれないよう、十分にご注意ください。

### 🚀 サーバーの仕様

本アプリは **Render** の無料プランを利用してデプロイしています。そのため、以下の仕様制限があります。

* **スリープ機能**: 15分間アクセスがないと、サーバーが自動的に休止状態（Spin down）に入ります。
* **初回起動の遅延**: スリープ状態からの復帰には、**30秒〜1分ほど**時間がかかります。
* **接続エラー時の対応**:
* ページが表示されない場合は、1分ほど待ってから再度読み込んでください。
* エラー画面が残ってしまう場合は、**ブラウザのキャッシュクリア**や**シークレットウィンドウ**でのアクセスをお試しください。
...
4. **自動リカバリー**: 4時間おきに実行されるバッチ処理が `Ready` タグのページを検出し、ADR を自動作成
